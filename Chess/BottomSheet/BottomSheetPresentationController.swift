...
public final class BottomSheetPresentationController: UIPresentationController {
    ...
    init(presentedViewController: UIViewController, presentingController: UIViewController?, dismissalHandler: BottomSheetModalDismissalHandler) {
        self.dismissalHandler = dismissalHandler
        super.init(presentedViewController: presentedViewController, presenting: presentingController)
    }
    ...
    private func endInteractiveTransition(verticalVelocity: CGFloat, verticalTranslation: CGFloat) {
        ...
        let finalProgress = (verticalTranslation - 0.5 * verticalVelocity * verticalVelocity / CGFloat(deceleration)) / presentedView.bounds.height
    ...
    public override func presentationTransitionDidEnd(_ completed: Bool) {
        if completed {
            ...
            setupScrollViewTrackingIfNeeded()
...
//    private func setPullBarFrame() { DEPECATED
//        pullBar?.frame.origin.y = (presentedView?.frame.minY ?? 0) - Style.pullBarHeight
//    }
    ...
    private func setupShadingView(containerView: UIView) {
        ...
        shadingView.backgroundColor = .black.withAlphaComponent(0.6)
    ...
    private func targetFrameForPresentedView() -> CGRect {
        ...
        let maxHeight = containerView.bounds.height - windowInsets.bottom
    ...
    private func setupScrollViewTrackingIfNeeded() {
        trackScrollView(inside: presentedViewController)
    }
    
    private func trackScrollView(inside viewController: UIViewController) {
        guard let scrollableViewController = viewController as? ScrollableBottomSheetPresentedController,
...
extension BottomSheetPresentationController: UIViewControllerAnimatedTransitioning {
    ...
    public func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        let offscreenFrame = CGRect(origin: CGPoint(x: 0, y: containerView.bounds.height), size: sourceView.frame.size)
...
extension BottomSheetPresentationController: UIScrollViewDelegate {
    ...
    public func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
        if didStartDragging {
            ...
            endInteractiveTransition(verticalVelocity: velocity.y, verticalTranslation: translation.y - lastContentOffsetBeforeDragging.y)
        } else {
            endInteractiveTransition(isCancelled: true)
        }
    ...
    private func shouldDragOverlay(following scrollView: UIScrollView) -> Bool {
        guard scrollView.isTracking, isInteractiveTransitionCanBeHandled else { return false }
